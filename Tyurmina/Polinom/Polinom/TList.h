...
public:
	...
	~TList();
	TNode<TKey, TData>* Search(TKey); //îïåðàöèÿ ïîèñêà
	void InsertBegin(TKey, TData); //âñòàâêà â íà÷àëî
	void InsertEnd(TKey, TData); //âñòàâêà â êîíåö
	void InsertAfter(TKey, TNode<TKey, TData>*);   //âñòàâêà ïîñëå ýëåìåíòà ñ çàäàíûì êëþ÷åì
	void InsertBefore(TKey, TNode<TKey, TData>*);  //äî ýëåìåíòà
	void Remove(TKey); //óäàëåíèå ýëåìåíòà ñ äàííûì êëþ÷åì
	bool IsEmpty() const; //ïðîâåðêà íà ïóñòîòó
	bool IsEnded() const; //äîøëè ëè ìû äî êîíöà ñïèñêà
	void Reset(); //óñòàíàâëèâàåò òåêóùèé óêàçàòåëü â íà÷àëî
	void Next();   //ïåðåäâèãàåò òåêóùèé óêàçàòåëü íà ñëåäóþùèé ýëåìåíò
	TNode<TKey, TData>* GetFirst() const; //âîçâðàùàåò ïåðâûé ýëåìåíò
	TNode<TKey, TData>* GetCurrent() const; //âîçâðàùàåò òåêóùèé ýëåìåíò
};
